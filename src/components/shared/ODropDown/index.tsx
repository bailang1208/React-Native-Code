...
const ODropDown = (props: Props) => {
  ...
  const [value, setValue] = React.useState(
    curIndex && items ? items[curIndex] : null,
  );
  ...
  return (
    <DropWrapper style={props.style}>
      <InnerWrapper onPress={onToggle}>
        {props.kindImage ? <KindIcon source={props.kindImage} /> : null}
        <SelLabel numberOfLines={1} ellipsizeMode={'tail'}>
          {value || props.placeholder}
        </SelLabel>
        <DropIcon
          style={{tintColor: props.dropIconColor || 'grey'}}
          source={COMP_ICONS.dropdown}
        />
      </InnerWrapper>
      {isOpen ? (
        <DropView>
